...
First, this is what the file looks like.
...
Start by opening a new file in your text editor and import the Python package `xml.etree.cElementTree`. Then add this block of code to generate the XML declaration and root node:
...
So we're off to a good start! Now we're going to write code to generate static pages like your home page, login, registration, etc.
...
If we were to run this code now, it would generate a file that looks like this:
...
For this part I'm using Django as my frontend to Python. Again, you can use whatever framework or language you want, the idea is still the same.
...
We've imported our database model `MyProductModel`, and since we're using Django, we updated our def with the `request` argument we added an HTTP response.

In short, we've made out sitemap.xml generator into a web page. I'll explain why further down. But first, let's write the code to generate all of our product pages in your CMS.
```
...
```
What we've done here is query our database for all "product" pages, then loop through the queryset to generate `<url>` nodes for every page. This could be 10 products or 1000 or even more.
...
For Google to read your sitemap it needs to be rendered as a static `.xml` file and available in your root as `www.your-website.com/sitemap.xml`. This means we need to add is as a `TemplateView` in our urls.py.
...
Okay, so we have a url pattern set up to serve our sitemap.xml, but how do we generate the file?
...
Let's create a url pattern called `generate_sitemap` and add it to the app urls.py:
...
- description: "Generate MySitemap CRON"
  ...
  retry_parameters:
    ...
    max_doublings: 5
```
This creates a Cron job for App Engine that will visit your page at `www.your-website/generate-sitemap/` which will then generate an updated sitemap.xml file that is accessible by Google and other search engines.

Push the Cron job to App Engine with this in terminal:
...
We don't want just anybody to have access, we only want our Cron job to access the page. But then, we can't give the Cron job admin access either.

So, we're going to use headers. This is safe because the `x-appengine-cron` header is only passed within Google's network, meaning it's unlikely to be spoofed.
